...
######################################### Classifier Results
prediction_file = pd.read_csv('PostRawActivity.csv')
video_path = Path(__file__).resolve().parents[0].joinpath('experiments','ActivityClassifierExperiment_1.csv')
video_data = pd.read_csv(video_path)
experiment_activities = ['EATING', 'WALK', 'STATIONARY', 'MOP', 'TRANSFER', 'OPENDISHWASHER', 'WIPE']
...
for times in range(len(activity_start_times)): 
    ...
    prediction_start = prediction_file.iloc[(prediction_file['Time (UTC)']- (activity_start_times[times]+timeshift)).abs().argsort()[:1]].index.tolist()
    prediction_end = prediction_file.iloc[(prediction_file['Time (UTC)']- (activity_end_times[times]+timeshift)).abs().argsort()[:1]].index.tolist()

    prediction_activities = sorted(prediction_file['Activity'][prediction_start[0]:prediction_end[0]].unique()) #Puts all activity labels predicted in a list
    predicted_labels = (prediction_file[prediction_start[0]:prediction_end[0]].groupby('Activity').size())#.tolist())
...
activity_categories = numpy.array(experiment_activities)
utils.make_confusion_matrix(matrix, categories=activity_categories, figsize=(20,20), percent=False, title="Confusion Matrix on Test Set")
quit()
